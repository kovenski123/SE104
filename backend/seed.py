...
def seed():
    ...
    try:
        ...
        # ============ MEMBERSHIPS ============
        from app.routers.memberships import _add_months
        today = date.today()
        mem = Membership(
            khach_hang_id=users[5].id,  # Trần Thị Quen
            loai_the=MembershipType.VANG,
            ngay_bat_dau=today - timedelta(days=10),
            ngay_ket_thuc=_add_months(today - timedelta(days=10), 6),
            phi_the=Decimal("2160000"),
            trang_thai="ACTIVE",
        )
        db.add(mem)
        ...
        sample_bookings = [
            ...
            (fields[2], users[5], today - timedelta(days=5), time(19, 0), time(20, 30)),
            (fields[1], users[4], today - timedelta(days=3), time(17, 0), time(18, 30)),
            (fields[3], users[5], today - timedelta(days=2), time(20, 0), time(21, 30)),
        ...
        print("  Khách VIP : quen@gmail.com / khach1234 (thẻ Vàng)")
    finally:
        db.close()
